import { useEffect, useState } from "react";

const Navbar = () => {
  const [show, setShow] = useState(false);
  const [scroll, setScroll] = useState(false);
  const handleClick = () => {
    setShow(!show);
    console.log(show);
  };
  let menuActive = show ? "left-0" : "-left-full";
  useEffect(() => {
    window.addEventListener("scroll", () => {
      if (window.scrollY > 5) {
        console.log("testis")
        setScroll(true);
        setShow(false);
      } else {
        setScroll(false);
      }
    });
  });

  let scrollActive = scroll ? "py-6 bg-white shadow" : "py-4";

  return (
    <div className={`navbar fixed w-full transition-all py-4 ${scrollActive}`}>
      <div className="container mx-auto px-auto ">
        <div className="navbar-box flex item-center justify-between">
          <div className="logo">
            <h1 className=" sm:text-2xl text-xl font-bold">home Service</h1>
          </div>
          <ul
            className={`flex lg:gap-12 md:static md:bg-transparent md:flex-row md:w-auto md:h-full md:translate-y-0 md:shadow-none md:text-black md:p-0 md:m-0 md:transition:none ${menuActive} gap-8 fixed left-0 top-1/2 -translate-y-1/2 flex-col px-9 py-6 rounded shadow-lg shadow-slate-300 bg-sky-400 font-bold text-white transition-all `}
          >
            <li className="bg-cyan-400 rounded-lg px-6 flex items-center gap-3 hover:bg-cyan-500 rounded-lg transition ease-in-out delay-100">
              <i class="ri-home-line text-3xl md:hidden block"></i>
              <a href="#" className="font-medium opacity-75">
                home
              </a>
            </li>
            <li className="bg-cyan-400 rounded-lg px-6 flex items-center gap-3 hover:bg-cyan-500 rounded-lg transition ease-in-out delay-100">
              <i class="ri-information-line text-3xl md:hidden block"></i>
              <a href="#" className="font-medium opacity-75" >
                ABOUT me
              </a>
            </li>
            <li className="bg-cyan-400 rounded-lg px-6 flex items-center gap-3 hover:bg-cyan-500 rounded-lg transition ease-in-out delay-100">
              <i class="ri-settings-3-line text-3xl md:hidden block"></i>
              <a href="#" className="font-medium opacity-75">
                service
              </a>
            </li>
            <li className="bg-cyan-400 rounded-lg px-6 flex items-center gap-3 hover:bg-cyan-500 rounded-lg transition ease-in-out delay-100">
              <i class="ri-image-line text-3xl md:hidden block"></i>
              <a href="#" className="font-medium opacity-75">
                project
              </a>
            </li>
          </ul>
          <div className="sosial flex items-center gap 2">
            <a
              href="#"
              className="bg-sky-400 px-5 py-2 rounded-full text-white font-bold hover:bg-sky-500 transition-all"
            >
              sosial media
            </a>
            <i
              class="ri-menu-fill text-3xl md:hidden block"
              onClick={handleClick}
            ></i>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Navbar;
